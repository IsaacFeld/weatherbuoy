...
document.addEventListener('click', (event) => {
    ...
    if(event.target.classList.contains('debug_copy')){
        let raw = document.querySelector('#raw_data')
        raw.select()
...
async function loadDebugData(){
    ...
    if(debugData.length == 0){
        request_container.innerHTML = `<p class="request_element" style="color: var(--seaside-50)";>No requests found, make a GET/POST request to <br> <span class="request_element" style="user-select: all;">bigwb.isaacf.me/debugger</span></p>`
    }
